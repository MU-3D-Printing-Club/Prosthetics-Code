...
// ****************** //
//  Battery Check Related Variables
//******************* //
int checkBatteryCounter = 0; // current count for deciding when to check the battery's voltage
int waitCycles = 100; // how long to wait until checking battery's voltage (unit: loopCycles)
enum pins
{
    THUMB_PIN = 3,
    INDEX_PIN = 5,
...
void loop() {
    ...
    //  Checks the battery voltage after count is greater than "waitCount" for setting LED status lights.
    if (checkBatteryCounter > waitCycles) {
        int currentVoltage = checkBatteryVoltage();
...
//  Description: Returns the calculated battery voltage
//******************************************************//
int checkBatteryVoltage(){
    // Battery Checking Related Variables
    int analogBatteryValue = 0;
    float batteryVoltage = 0;
    //Convertion Factor from analog to volts
    float voltageConversionFactor = 0.0048;
    Serial.println("Checking Battery Voltage...\n");
    analogBatteryValue = analogRead(A0);
    batteryVoltage = analogBatteryValue * voltageConversionFactor;

    //Return "-1" as an error indicator that the voltage is out of acceptable range
    if ( batteryVoltage < 0.5 || batteryVoltage > 6 ){
        return -1;
    }
